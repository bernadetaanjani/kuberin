...
class Service_summarize_new extends CI_Controller {
    public $stopwords;
    public $num_docs;
    ...
    function getDataBeritaSummarize()
    {
        ...
        $berita = $this->Summarizemodel->getBeritaSummarizeNew($katakunci,$tanggalrange,$sumber,$limitget);
        if(!empty($berita))
        {
            echo json_encode(array('st'=>'1','msg'=>$berita,'katakunci'=>$katakunci));
        }
        else
        {
    ...
    function summarizeberita()
    {
        ...
        $idberitasum = $_POST['id_beritatmp'];
        $androidID=$_POST['android_id'];
        $androidName=$_POST['nama_device'];
        $datetimesummarize=$_POST['datetime_summarize'];

        //$berita = $this->Summarizemodel->getBeritaSummarize($katakunci,$tanggalrange,$sumber);
        $tmpidber = explode(',', $idberitasum);
        foreach ($tmpidber as $idberitaget) {
...
//                    $rowssumtmp=substr($rowssum,500);
            ...
            echo json_encode(array('st' => '1', 'msg' => $data, 'sum_score'=>$summarybaru, 'berita' => $tampberitaparsex));
