...
class Service_summarize extends CI_Controller {
    public $stopwords;
    public $num_docs;
    ...
    function sumber()
    {
        $data = $this->Summarizemodel->getSumberBerita();
        header('Content-Type: application/json');
        if($data!=null)
        {
            echo json_encode(array('st'=>'1','sumber'=>$data));
        }
        else
        {

            echo json_encode(array('st'=>'0','sumber'=>'kosong'));
        }

    }
//    function getDataBeritaSummarize()
//    {
//        $katakunci = $_POST['katakunci'];
//        $tanggalrange = $_POST['tanggalrange'];
//        $sumber = $_POST['optsumberberita'];
//        $limitget = $_POST['optjumlahberita'];
//        $berita = $this->Summarizemodel->getBeritaSummarize($katakunci,$tanggalrange,$sumber,$limitget);
//        if(!empty($berita))
//        {
...
//    }
    function summarizeberita()
    {
        ...
        $androidName=$_POST['nama_device'];
        $datetimesummarize=$_POST['datetime_summarize'];
...
//        $androidName=$this->input->post('nama_device');
//        $datetimesummarize=$this->input->post('datetime_summarize');
        ...
        if(!empty($berita))
        {
            ...
            //$data = implode(' ',$this->multi_summary($this->corpus_summary_temp));
...
//                    $rowssumtmp=substr($rowssum,500);
            ...
            foreach ($tmpkal as $nganukal) {
                ...
                $tmpIDber[] = $nganukal['id'];
            }
            $tmpimpID = implode(",", $tmpIDber);

            $tampLog = array(
                'datetime_summarize' => $datetimesummarize,
                'keyword' => $katakunci,
                'summarize' => $data,
                'id_device' => $androidID,
                'nama_device' => $androidName,
                'idberita_terkait' => $tmpimpID
            );
            $idlog = $this->Summarizemodel->insertLog($tampLog);


            foreach ($summarybaru as $sumbro) {
                $tampDetailLog = array(
                    'kalimat' => $sumbro['kalimat'],
                    'score' => $sumbro['score'],
            ...
            echo json_encode(array('st' => '1', 'msg' => $data, 'sum_score'=>$summarybaru, 'berita' => $tampberitaparse));
        }
        else
        {
            echo json_encode(array('st' => '0', 'msg' => 'Data Kosong'));
