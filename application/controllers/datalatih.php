...
class Datalatih extends CI_Controller {
    ...
    public $stopwords = array('--', '-', 'ada','adalah','adanya','adapun','agak','agaknya','agar','akan','akankah','akhir','akhiri','akhirnya','aku','akulah','amat','amatlah','anda',
        ...
        {       
                // $data['news'] = $this->datalatihmodel->get_news();
                // $config['base_url']='http://localhost/kuberin/index.php/datalatih';
                $config['total_rows']=$this->datalatihmodel->record_count();
                $config['per_page']=5;
                ...
                $config['base_url']=base_url()."index.php/datalatih/index";
                $config['total_rows']=$this->datalatihmodel->record_count();
                $config['per_page'] = 10;
                ...
                $data['data_latih']  = $this->datalatihmodel->get_news($config["per_page"], $page, false);
                $data['links'] = $this->pagination->create_links();
                ...
                $data['nav_datalatih']='active';
                $data['nav_naivebayes']='';
                $data['page_title']='Kuberin Administrator';
                $this->load->view('header', $data);
                $this->load->view('navigation', $data);
                $this->load->view('datalatih_view', $data);
                $this->load->view('footer', $data);
        }

        public function view($slug)
        {
                $data = $this->news_model->get_news_item($slug);
                $data['analyzed']=$this->news_model->check_analyzed($slug);
                // var_dump($data['analyzed']);
                ...
                $this->load->view('news/view',  array('data'=>$data));
