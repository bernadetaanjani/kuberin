...
//            }
            ...
            $this->load->view('summarize_view', $data);
...
//    }
    ...
    function summarizeberita()
    {
        ...
        $sumberjadi=implode(', ',$sumber);//gabungin

        //$this->form_validation->set_rules('katakunci', 'Kata Kunci', 'trim|required|xss_clean');
//        $this->form_validation->set_rules('optjumlahberita', 'Kata Kunci', 'trim|required|xss_clean');
        //if($this->form_validation->run()==FALSE)
        if(!empty($katakunci)&&!empty($limitget))
        {
            $berita = $this->Summarizemodel->getBeritaSummarize($katakunci,$tanggalrange,$sumberjadi, $limitget);
            if(!empty($berita))
            {
                foreach($berita as $list_berita)
                {
                ...
                $sum_temp = array();
                foreach ($this->Doc as $doc) {
                    $doc_temp = $this->single_summary($doc);//summary satu berita
                    $this->total_document++; //menghitung jumlah dokumen
                    array_push($sum_temp, implode(' ',$doc_temp));
                    $this->corpus_summary_temp = $this->corpus_summary_temp.implode(' ',$doc_temp).' ';
                    //echo $this->corpus_summary_temp;
                    array_push($this->corpus_summary,implode(' ',$doc_temp));//terpakai
                    //print_r($doc_temp);
                }
                ...
                //print_r($tmps);
                $summary = $this->summary_sentences_score;//multi summary
