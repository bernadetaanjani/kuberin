...
class Cronberitamenunggu extends CI_Controller {
    ...
    function index()
    {
        $taggetberita = $this->Scrapmodel->GetUrlBeritaMenunggu();
        $tampungberita = array();
        $tampunggetberita = array();
        $datagetberitafix = array();
        $tmpluar = array();
        $gabungurl = array();
        ...
        if (!empty($taggetberita)) {
            foreach ($taggetberita as $rows) {
                $time_start = microtime(true);
                $urlberita = $rows['url_berita'];
                $idgeturl = $rows['tbl_geturl_id_geturl'];
                ...
                $ber = file_get_html($urlberita);
                $tampungurl = explode('/', $urlberita);
...
//                        $tampindexisifixa = $tampungpecahisifixa[0].'.'.$tampungpecahisifixa[1];
                ...
                if ((!empty($judulfix) && $judulfix !== ' ') && (!empty($kategorifix) && strpos($kategorifix, 'empty') == false) && (!empty($tanggal) && $tanggal !== '//') && (!empty($waktu)) && (!empty($isiberitafix)))
                {

                    $result = $this->Scrapmodel->CheckDbBerita($urlberita,$judulfix);
                    if (!$result) {
                        $tmpber = array(
                            'judul' => $judulfix,
                            'tanggal' => $tanggal,
                            ...
                            'gambar' => $gambar,
                            'isi_berita' => $isiberitafix,
                            'link' => $urlberita,
                            'sumberberita' => $sumberberita,
                            'timer' => $time,
                            'datetime_scraping' => $timenow
                        );
                        $this->Scrapmodel->InsertBerita($tmpber);
                        $tmpber = array();
                        $tmpgetberita = array(
                            'url_berita' => $urlberita,
                            'status' => 'sudah',
                            'tbl_geturl_id_geturl' => $idgeturl
                        );
                        $this->Scrapmodel->SetStatusTagUrlBerita($tmpgetberita, $urlberita);
                        $tmpgetberita = array();
                    }
